...
EOF
    ...
    psql -At -h ${PG_HOST} -U ${PG_SUPERUSER} -p ${PG_PORT} "dbname=${PG_DBNAME} sslmode=disable" -c "CHECKPOINT" > /dev/null 2>&1
    psql -At -h ${PG_HOST} -U ${PG_SUPERUSER} -p ${PG_PORT} "dbname=${PG_DBNAME} sslmode=disable" -c "VACUUM ANALYZE" > /dev/null 2>&1
    psql -At -h ${PG_HOST2} -U ${PG_SUPERUSER} -p ${PG_PORT} "dbname=${PG_DBNAME} sslmode=disable" -c "CHECKPOINT" > /dev/null 2>&1
    psql -At -h ${PG_HOST2} -U ${PG_SUPERUSER} -p ${PG_PORT} "dbname=${PG_DBNAME} sslmode=disable" -c "VACUUM ANALYZE" > /dev/null 2>&1
    ...
    catchup_time=$(psql -At -h ${PG_HOST} -U ${PG_SUPERUSER} -p ${PG_PORT} "dbname=${PG_DBNAME} sslmode=disable" -c "SELECT EXTRACT(epoch from catchup_time) FROM xdb_monitor_repl_catchup_time() WHERE xdb_slot_name LIKE 'xdb%';")
